    ...
    <div style={{ border: `1px solid ${T.dwGrey}`, padding: 10 }}>
      <div style={{ display: "flex", alignItems: "center", gap: 6, minWidth: 0 }}>
        ...
        <PixelBtn
          small color="red"
          disabled={called}
          title={called ? "No se puede eliminar: hay al menos un sitio de llamada activo a este FC" : "Eliminar este FC"}
...
// los FCs (crear/renombrar/eliminar + su interfaz IN/OUT) — el bloque Main
// no aparece aquí, se edita siempre desde el editor central.
export default function BlocksPanel({ blocks, onAddBlock, onRenameBlock, onRemoveBlock, onAddParam, onRenameParam, onRemoveParam }) {
  ...
  return (
    <div>
      ...
      {fcs.length === 0 && (
        <span style={{ fontSize: 12, color: "#888", fontStyle: "italic" }}>Aún no hay ningún FC — crea uno con el botón de abajo.</span>
      )}
      <div style={{ display: "flex", flexDirection: "column", gap: 10, marginTop: fcs.length ? 0 : 10 }}>
        {fcs.map((block) => (
          <BlockCard
            key={block.id}
            block={block}
      ...
      <div style={{ marginTop: 10 }}>
        <PixelBtn small color="dwGrey" onClick={onAddBlock}>+ Nuevo FC</PixelBtn>
      </div>
    </div>
  );
}
